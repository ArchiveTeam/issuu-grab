...
LUA_SHA1 = get_hash(os.path.join(CWD, 'issuu.lua'))
...
class WgetArgs(object):
    ...
    def realize(self, item):
        ...
        for item_name in item['item_name'].split('\0'):
            ...
            if item_type == 'issue':
                user, id_, name = item_value.split(':', 2)
                wget_args.extend(['--warc-header', 'issuu-issue-name: '+name])
                wget_args.extend(['--warc-header', 'issuu-issue-id: '+id_])
                wget_args.append('https://issuu.com/{}/docs/{}'.format(user, name))
            elif item_type == 'user':
                wget_args.extend(['--warc-header', 'issuu-user: '+item_value])
                wget_args.append('https://issuu.com/' + item_value)
            else:
...
project = Project(
    title='issuu',
    project_html='''
        <img class="project-logo" alt="Project logo" src="https://wiki.archiveteam.org/images/thumb/d/d5/Issuu-icon.png/600px-Issuu-icon.png" height="50px" title=""/>
        <h2>issuu.com <span class="links"><a href="https://issuu.com/">Website</a> &middot; <a href="http://tracker.archiveteam.org/issuu/">Leaderboard</a></span></h2>
        <p>Archiving issuu.</p>
    '''
)
...
pipeline = Pipeline(
    ...
    GetItemFromTracker('http://{}/{}/multi={}/'
        .format(TRACKER_HOST, PROJECT_ID, MULTI_ITEM_SIZE),
        downloader, VERSION),
